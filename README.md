...
```
The second is provided by a python script, plot_results.py, for running by itself from the CytoSPACE outputs. There are four parameters than need to be set by users: -ctap (cell_type_assignments_by_spot), -alp (assigned_locations_path), -ncip (new_cell_index_path) and -cp (coordinates_path). This script can be run from the command line for the breast cancer:
```bash
python plot_results.py -ctap cytospace_results/cell_type_assignments_by_spot.csv -alp cytospace_results/assigned_locations.csv -ncip cytospace_results/new_cell_type.csv -cp  brca_STdata_coordinates.txt
```
and melanoma: 
```bash
python plot_results.py -ctap cytospace_results/cell_type_assignments_by_spot.csv -alp cytospace_results/assigned_locations.csv -ncip cytospace_results/new_cell_type.csv -cp  melanoma_STdata_slide1_coordinates.txt -nr 5 -nc 3 -ss 1100 -pm s -pv
```
