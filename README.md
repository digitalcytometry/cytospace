...
### Solver options
CytoSPACE provides three solver options. In short, we recommend using the default option `lapjv` if your system supports AVX2 (you were able to successfully install it with `pip install lapjv`) and `lap_CSPR` otherwise. To use `lap_CSPR` instead, pass `-sm lap_CSPR` to your `cytospace` call.
